...
import { ArrowRightOutlined, DatabaseOutlined, DownOutlined, SafetyCertificateOutlined, UpOutlined, WalletOutlined } from '@ant-design/icons';
...
/** 聚合供应商的固定接入入口，不主动查询余额或发起授权。 */
export default function TokenDanceRecommendedCard({ provider, loading, modelCount, enabledModelCount, onOpen, onSelect, expanded, onToggle }: Props) {
  return (
    <section className="tokendance-recommended" aria-labelledby="tokendance-title" aria-busy={loading}>
      <div className="tokendance-recommended__summary">
        <div className="tokendance-recommended__identity">
          <img className="tokendance-recommended__mark" src={tokenDanceLogo} alt="" width={40} height={40} />
          <div>
            <div className="tokendance-recommended__title">
              <h2 id="tokendance-title">TokenDance</h2>
              <span className="tokendance-recommended__badge">聚合供应商</span>
            </div>
            <span className="tokendance-recommended__description">一个账户，连接多品牌 AI 模型</span>
          </div>
        </div>
        ...
        <button type="button" className="tokendance-recommended__toggle" aria-expanded={expanded}
          aria-controls="tokendance-aggregator-details" onClick={onToggle}>
          {expanded ? '收起' : '展开详情'} {expanded ? <UpOutlined aria-hidden="true" /> : <DownOutlined aria-hidden="true" />}
        </button>
      </div>
      <div id="tokendance-aggregator-details" className="tokendance-recommended__details" hidden={!expanded}>
        ...
        {!loading && !provider && <Alert type="warning" showIcon message="未找到聚合供应商配置" description="请确认已执行 060-tokendance-default-provider.sql，并检查该供应商是否被删除。无需手动新增或填写密钥。" />}
      </div>
    </section>
  );
}
